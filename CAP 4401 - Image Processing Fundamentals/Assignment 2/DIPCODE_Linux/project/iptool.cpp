	...
	while(fgets(str,MAXLEN,fp) != NULL) {
		...
		for (int s = 0; s < roiCount; s++) {
			...
			if (strncasecmp(pch,"add",MAXLEN)==0) {
				...
				/* Alternate Histogram Stretching */
				int A = atoi(strtok(NULL, " "));
				int B = atoi(strtok(NULL, " "));

				utility::althistostretch(src, tgt, A, B);
			}
				...
				int A = atoi(strtok(NULL, " "));
				int B = atoi(strtok(NULL, " "));
				utility::histothres(src, tgt, threshold, bf, A, B);
			}
				...
				int A = atoi(strtok(NULL, " "));
				int B = atoi(strtok(NULL, " "));
				utility::colorstretch(src, tgt, c, A, B);
			}
			else if (strncasecmp(pch,"hsistretch",MAXLEN)==0){

				/* HSI Histogram Stretching - Single*/
				char c[2];
				pch = strtok(NULL, " ");
				strcpy(c, pch);
				int A = atoi(strtok(NULL, " "));
				int B = atoi(strtok(NULL, " "));
				utility::hsistretch(src, tgt, c, A, B);
			}
