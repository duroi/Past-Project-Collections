...
*/
bool CountdownTree::contains(int x) const{//not optional: Created in-lab
...
Node* Node::search(int x){//optional: Created in-lab
    if(x == this->data){
        return this;
    }else if(x > this->data){
...
void CountdownTree::insert(int x){//REQUIRED COUNTDOWNTREE INSERT: Created in-lab
...
void Node::insert(int x){//optional NODE INSERT: Created in-lab
...
void Node::rebalance(){//required NODE REBALANCE: Created in-lab
...
void Node::updateHeight(){//OPTIONAL: Created in-lab
...
Node* Node::removeAllLessThan(int x) {
    ...
    if (this != nullptr){
        ...
        if (this->data < x){
            ...
            if (this->left != nullptr){
                this->left = this->left->removeAllLessThan(k);
                this->left->parent = this;
            }
                return this;
        }
...
void CountdownTree::rotateToRoot(int x){
    

    Node* temp = root->search(x);
    ...
    if(temp != nullptr){
        ...
        //is right child
            if(temp->parent != nullptr && temp->parent->right == temp) {
                temp->rotateRight();
            }
        //is left child
            if(temp->parent != nullptr && temp->parent->left == temp){
                temp->rotateLeft();
            }
        }
    }
